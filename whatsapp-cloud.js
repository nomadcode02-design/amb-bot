...
// Marca un mensaje ENTRANTE como leído. Esto no es solo prolijidad (el
// "visto" azul) — es lo que hace que Meta termine de "activar" la
// conversación del lado de ellos para el PRIMER mensaje de un contacto
// nuevo. Sin esto, la API puede aceptar el envío de la respuesta (devuelve
// success) pero no entregarlo de verdad hasta que el contacto escribe una
// segunda vez, porque la ventana recién queda del todo abierta ahí.
async function marcarComoLeido(messageId) {
  ...
  try {
    ...
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
      console.error(`⚠️ No se pudo marcar como leído el mensaje ${messageId}:`, data.error?.message || res.status);
    }
  } catch (e) {
    console.error(`⚠️ Error marcando como leído el mensaje ${messageId}:`, e.message);
  }
}
...
async function sendMessage(numero, texto) {
  ...
  try {
    const data = await encolarEnvio(numeroLimpio, async () => {
      await delayAleatorio();
      console.log(`📤 Enviando mensaje a: ${numeroLimpio}`);
      const resultado = await enviarTextoReal(numeroLimpio, texto);
      console.log(`✅ Meta aceptó el envío a ${numeroLimpio} (todavía no confirma entrega real)`);
      return resultado;
    });
    const waMessageId = data?.messages?.[0]?.id || null;
    return { enviado: true, jid: numeroLimpio, waMessageId };
  } catch (e) {
    if (e.code === 131047) {
