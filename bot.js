...
const MAX_INTENTOS_RECONEXION = 6; // después de esto, deja de generar QR solo
...
// Delay largo antes de contestar (1-2 minutos): un bot que responde al
// instante es una de las señales más obvias de automatización. Esto hace
// que la confirmación tarde en llegar, como si alguien la estuviera
// atendiendo manualmente en vez de un programa.
function delayRespuestaHumana() {
  const ms = 60000 + Math.random() * 60000; // 60-120s
  return new Promise(resolve => setTimeout(resolve, ms));
}
...
// ---------- Modo cauteloso (permanente) ----------
// Cooldowns duplicados y topes de mensajes a la mitad, todo el tiempo. Con
// el volumen real de esta barbería (~16 turnos/día, un solo mensaje por
// turno) no se nota en la velocidad, y da un margen extra de seguridad
// contra restricciones de WhatsApp de forma constante, sin depender de
// activarlo/desactivarlo a mano.
function enModoCauteloso() {
  return true;
}
...
function envioDentroDeTopes() {
  ...
  const cauteloso = enModoCauteloso();
  const topeHora = cauteloso ? Math.ceil(MAX_MENSAJES_POR_HORA / 2) : MAX_MENSAJES_POR_HORA;
  const topeDia = cauteloso ? Math.ceil(MAX_MENSAJES_POR_DIA / 2) : MAX_MENSAJES_POR_DIA;
  ...
  if (enUltimaHora >= topeHora) {
    console.warn(`🛑 Tope de seguridad${cauteloso ? ' (modo cauteloso)' : ''}: ${enUltimaHora} mensajes en la última hora (máx ${topeHora}). Pausando envíos, el bot sigue conectado.`);
    return false;
  }
  if (enUltimoDia >= topeDia) {
    console.warn(`🛑 Tope de seguridad${cauteloso ? ' (modo cauteloso)' : ''}: ${enUltimoDia} mensajes en las últimas 24hs (máx ${topeDia}). Pausando envíos, el bot sigue conectado.`);
    return false;
  }
  return true;
}
...
function encolarEnvio(jid, fn) {
  colaEnvios = colaEnvios.then(async () => {
    ...
    const cauteloso = enModoCauteloso();
    const multiplicador = cauteloso ? 2 : 1;

    // Esperar el cooldown global (desde el último envío, sea a quien sea), con jitter
    let espera = jitter(COOLDOWN_GLOBAL_MS * multiplicador, COOLDOWN_GLOBAL_JITTER_MS) - (ahora() - ultimoEnvioGlobal);
    ...
    espera = jitter(COOLDOWN_CONTACTO_MS * multiplicador, COOLDOWN_CONTACTO_JITTER_MS) - (ahora() - ultimoAEsteJid);
    if (espera > 0) {
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      if (intentosReconexion >= MAX_INTENTOS_RECONEXION) {
        // Se alcanzó el tope de reintentos: dejamos de generar QR y de
        // intentar reconectar solo. El servidor (panel, API de turnos,
        // etc.) sigue corriendo normal, no depende de esto. Para volver a
        // intentar conectar el bot hay que reiniciar el servicio (ej. un
        // redeploy en Railway), lo cual arranca startBot() de nuevo desde
        // cero con el contador en 0.
        latestQR = null;
        console.log(`🛑 Se alcanzaron ${MAX_INTENTOS_RECONEXION} intentos de reconexión sin éxito. Dejo de reintentar y de generar QR — el panel sigue funcionando normal. Reiniciá el servicio para volver a intentar.`);
        return;
      }
      ...
      console.log(`🔁 Reintentando conexión en ${Math.ceil(espera / 1000)}s (intento ${intentosReconexion}/${MAX_INTENTOS_RECONEXION})`);
      setTimeout(startBot, espera);
    } else if (connection === 'open') {
      isReady = true;
      latestQR = null;
      intentosReconexion = 0;
      console.log('✅ BOT CONECTADO EXITOSAMENTE Y LISTO (modo cauteloso permanente: cooldowns dobles, topes a la mitad)');
    }
  });
  ...
  sock.ev.on('messages.upsert', async (event) => {
    ...
    for (const msg of event.messages) {
      ...
      try {
        ...
        console.log(`⏳ Esperando un rato antes de contestarle a ${jidDestino} (simula que alguien lo está atendiendo)`);
        await delayRespuestaHumana();
