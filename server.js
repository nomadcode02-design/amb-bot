...
function contarCortesConfirmados(numeroCliente) {
  ...
  return leerTurnos().filter(t => t.completado && limpiarNumero(t.whatsapp) === numeroNormalizado).length;
}
...
const FIDELIDAD_TARJETA_ACTIVA = false;
...
// fidelidad de un cliente por su número. DESACTIVADO por pedido del
// cliente (todavía no lo necesita) — la lógica real queda abajo comentada,
// lista para reactivar con solo sacar el "return" de arriba.
app.get('/api/fidelidad', (req, res) => {
  return res.status(503).json({ error: 'La consulta de tarjeta de fidelidad todavía no está disponible.' });

  /* ---- lógica real, comentada hasta reactivar ----
  ...
  ---- fin de la lógica real comentada ---- */
});
