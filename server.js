...
const { sendMessage, armarLinkWhatsApp, limpiarNumero, marcarComoLeido } = require('./whatsapp-cloud');
...
// ---------- Aviso de fidelidad, sumado al mensaje de confirmación ----------
// No se manda un mensaje aparte para avisar que activó la tarjeta o ganó un
// premio, porque en ese momento (cuando el barbero tilda el ✓ en el panel)
// la ventana de 24hs casi seguro está cerrada. En cambio, se le suma esta
// info al mensaje de confirmación de SU PRÓXIMA reserva — ahí la ventana
// está garantizada, porque él mismo le acaba de escribir al bot.
// La tarjeta arranca de cero de verdad: no cuenta NINGÚN corte de antes de
// esta fecha, ni siquiera los que ya estaban confirmados. Si el lanzamiento
// termina siendo otro día, se cambia solo este valor (y el mismo en
// panel.html, tienen que coincidir los dos).
const FIDELIDAD_FECHA_INICIO = '2026-09-01';

function contarCortesConfirmados(numeroCliente) {
  const numeroNormalizado = limpiarNumero(numeroCliente);
  return leerTurnos().filter(t =>
    t.completado &&
    limpiarNumero(t.whatsapp) === numeroNormalizado &&
    t.dia >= FIDELIDAD_FECHA_INICIO
  ).length;
}

const CORTES_PARA_ACTIVAR_TARJETA = 3; // igual que en panel.html — no tocar sin cambiar los dos

// Interruptor único para apagar TODO lo de la tarjeta de fidelidad (el
// aviso por WhatsApp de acá abajo). Para reactivarla: FIDELIDAD_TARJETA_ACTIVA = true.
const FIDELIDAD_TARJETA_ACTIVA = true;

function mensajeHitoFidelidad(totalCortesPrevios) {
  if (!FIDELIDAD_TARJETA_ACTIVA) return '';
  if (totalCortesPrevios === CORTES_PARA_ACTIVAR_TARJETA) {
    return `\n\n🎉 ¡Activaste tu *tarjeta de fidelidad*! Cada 10 cortes: 50% off en el 5° y un corte GRATIS en el 10°.`;
  }
  if (totalCortesPrevios < CORTES_PARA_ACTIVAR_TARJETA) return '';

  // Los cortes usados para desbloquear la tarjeta no cuentan como
  // casilleros — el ciclo de 10 arranca recién después de activarla.
  const cortesParaCiclo = totalCortesPrevios - CORTES_PARA_ACTIVAR_TARJETA;
  const resto = cortesParaCiclo % 10;
  if (resto === 5) {
    return `\n\n🎁 ¡Este turno te toca con *50% de descuento* (en cualquier servicio)! Se lo mostrás al barbero.`;
  }
  if (resto === 0 && cortesParaCiclo > 0) {
    return `\n\n🎁 ¡Este turno te toca *GRATIS*! Completaste tu tarjeta de fidelidad.`;
  }
  return '';
}
...
// y lo manda completo.
// Además, ignora turnos que ya se marcaron como completados en el panel o
// que ya pasaron de horario — si un cliente escribe después de eso, no
// tiene sentido volver a mandarle una confirmación vieja.
function buscarTurnoPorNumero(numeroCliente) {
  const ahora = new Date();
  const numeroClienteNormalizado = limpiarNumero(numeroCliente);
  const turnos = leerTurnos()
    .filter(t => !t.completado && fechaHoraDelTurno(t) > ahora)
...
app.post('/api/reservar', async (req, res) => {
  try {
    ...
    // La confirmación al cliente SOLO se manda cuando el bot lee su
    // mensaje real (webhook, más abajo) — nunca antes. Un intento
    // automático acá, antes de que escriba, siempre fallaría con "Re-
    // engagement message" (ventana de 24hs cerrada) para cualquier
    // cliente nuevo, así que no tiene sentido intentarlo.
...
// Endpoint público (sin clave): consulta el estado de la tarjeta de
// fidelidad de un cliente por su número. Usa EXACTAMENTE la misma cuenta
// que mensajeHitoFidelidad (contarCortesConfirmados), así que ya respeta
// solo también la fecha de corte (FIDELIDAD_FECHA_INICIO) — no hace falta
// tocar nada más acá para que el arranque en cero sea consistente.
app.get('/api/fidelidad', (req, res) => {
  const { telefono } = req.query;
  if (!telefono) {
    return res.status(400).json({ error: 'Falta el número de teléfono.' });
  }
  try {
    const cortes = contarCortesConfirmados(telefono);
    const activo = cortes >= CORTES_PARA_ACTIVAR_TARJETA;

    if (!activo) {
      return res.json({
        cortes,
        activo,
        resto: cortes, // posición a resaltar en la grilla mientras se activa
        proximoPremio: null,
        faltan: CORTES_PARA_ACTIVAR_TARJETA - cortes,
      });
    }

    const cortesParaCiclo = cortes - CORTES_PARA_ACTIVAR_TARJETA;
    const restoCiclo = cortesParaCiclo % 10;

    let proximoPremio, faltan, resto;
    if (cortesParaCiclo > 0 && restoCiclo === 0) {
      // Justo completó el ciclo (le tocó GRATIS este último corte) — se ve
      // como el casillero 10 lleno, y el próximo objetivo es el 50% del
      // ciclo siguiente.
      resto = 10;
      proximoPremio = '50% de descuento';
      faltan = 5;
    } else if (restoCiclo < 5) {
      resto = restoCiclo;
      proximoPremio = '50% de descuento';
      faltan = 5 - restoCiclo;
    } else {
      resto = restoCiclo;
      proximoPremio = 'un corte gratis';
      faltan = 10 - restoCiclo;
    }

    res.json({ cortes, activo, resto, proximoPremio, faltan });
  } catch (e) {
    console.error('Error consultando fidelidad:', e);
    res.status(500).json({ error: 'No se pudo consultar la tarjeta.' });
  }
});
...
app.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    // Marcar como leído ANTES de contestar: esto es lo que hace que Meta
    // active del todo la conversación con contactos nuevos (ver comentario
    // en whatsapp-cloud.js). Lo esperamos con await para que el "read"
    // llegue antes que la respuesta.
    ...
    if (turno && !turno.confirmacionEnviada) {
      // Primera vez que este número escribe sobre su turno vigente:
      // mandamos la confirmación y la marcamos, para no repetirla después.
      turno.confirmacionEnviada = true;
      guardarTodosLosTurnos(leerTurnos().map(t => (t.id === turno.id ? turno : t)));

      // El conteo se toma ANTES de este turno (todavía no pasó, así que no
      // suma) — si activó la tarjeta o ganó un premio en su última visita
      // confirmada, se lo avisamos acá, aprovechando que la ventana está
      // abierta.
      const cortesHastaAhora = contarCortesConfirmados(numeroCliente);
      respuesta = construirMensajeConfirmacion(turno) + mensajeHitoFidelidad(cortesHastaAhora);
    }
    // Si no hay turno vigente para este número (nunca reservó, ya lo
    // completaron en el panel, o ya pasó de horario), o si ya se le mandó
    // la confirmación antes, el bot no responde nada.
