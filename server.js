...
// Varias versiones del mensaje de confirmación, elegidas al azar: mismos
// datos, mismo formato general, pero redacción distinta cada vez — igual
// idea que con los recordatorios, para no repetir un texto idéntico.
function construirMensajeConfirmacion(turno) {
  ...
  const variantes = [
    ...
    `💈 *AMB BARBERS - Confirmación de turno*\n\n` +
    `¡Hola ${turno.nombre}! Quedó reservado tu turno:\n\n` +
    `Barbero: ${turno.barbero} 💈\n` +
    `Servicio: ${turno.servicio} ✂️\n` +
    `Día: ${fechaLinda} 📅\n` +
    `Hora: ${turno.horario} hs 🕐\n` +
    `Precio: ${turno.precio} 💰\n\n` +
    `Te esperamos en Calle 9 de Julio, entre Mitre y Av. Ramón Barrera, Santa Rosa - 25 de Mayo, San Juan. 📍\n\n` +
    `Llegando 5 minutos antes ya te podés acomodar tranquilo.\n\n` +
    `¡Nos vemos!`,

    `✅ AMB BARBERS confirma tu turno, ${turno.nombre}!\n\n` +
    `📅 ${fechaLinda} - 🕐 ${turno.horario} hs\n` +
    `💈 Con ${turno.barbero}\n` +
    `✂️ ${turno.servicio}\n` +
    `💰 ${turno.precio}\n\n` +
    `📍 Nos encontrás en Calle 9 de Julio, entre Mitre y Av. Ramón Barrera, Santa Rosa - 25 de Mayo, San Juan.\n\n` +
    `Un consejo: llegá 5 minutitos antes para arrancar sin apuro.\n\n` +
    `¡Te esperamos! 💈`,
  ];
...
app.post('/api/reservar', async (req, res) => {
  try {
    ...
    // REGLA DE ORO para no arriesgar el número: el bot NUNCA inicia una
    // conversación por su cuenta. Si el cliente ya nos escribió hace poco
    // (la ventana de WhatsApp sigue abierta), esto le manda la confirmación
    // directo. Si no, sendMessage no manda nada acá — va a esperar a que
    // el cliente le escriba desde el link (ver setOnMensaje más abajo), que
    // es la única forma en que el bot puede empezar a hablarle a alguien.
...
// confirmar y le contestamos con los datos reales de SU turno — una sola
// vez por turno (queda marcado confirmacionEnviada). Si ya se le mandó esa
// confirmación antes, o no tiene ningún turno pendiente, el bot no le
// contesta nada más. El único otro mensaje que puede llegar a recibir es
// el recordatorio automático del día del turno (eso es aparte, por cron).
