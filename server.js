...
function registrarMensaje({ numero, texto, tipo, enviado, error }) {
  try {
    ...
    // Recortamos por delante, nos quedamos con los últimos N
...
async function enviarYRegistrar(numero, texto, tipo) {
  try {
    ...
    registrarMensaje({ numero, texto, tipo, enviado: resultado.enviado, error: resultado.enviado ? null : 'ventana de 24hs cerrada' });
    return resultado;
  } catch (e) {
...
app.post('/api/reservar', async (req, res) => {
  try {
    ...
    // REGLA: la confirmación al cliente SOLO se manda cuando él le escribe
    // al bot (desde el link de WhatsApp del formulario) — nunca automático
    // al crear la reserva, aunque la ventana ya esté abierta. Eso pasa en
    // el webhook, más abajo en este archivo.
